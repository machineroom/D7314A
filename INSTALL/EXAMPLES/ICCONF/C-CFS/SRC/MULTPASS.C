...
/*{{{  DeleteChannel */
static void DeleteChannel (Channel *Channels[], int ChannelsSize, int ChannelsIndex)
{
    RotateChannels(Channels, ChannelsSize, ChannelsIndex + 1);

    Channels[ChannelsSize] = Channels[ChannelsSize - 1]; /* NULL terminated */
    Channels[ChannelsSize - 1] = NULL;
}
/*}}}*/

/*{{{  InsertChannel */
static void InsertChannel (Channel *Channels[], int ChannelsSize, int ChannelsIndex)
{
    Channels[ChannelsSize - 1] = Channels[ChannelsSize]; /* NULL terminated */
    Channels[ChannelsSize] = NULL;
    
    RotateChannels(Channels, ChannelsSize, ChannelsSize - (ChannelsIndex + 1));
}
/*}}}*/
...
int main (void)
{
    ...
    if (InputSize == OutputSize)
    {
        ...
        {
            /*{{{   */
            int InputIndex = ProcAltList(Input);
            
            Value = ChanInInt32(Input[InputIndex]);
            
            DeleteChannel(Input, InputSize, InputIndex);
            DeleteChannel(Output, OutputSize, InputIndex);
            
            Value = IOProcesses(Input, InputSize - 1, Output, OutputSize - 1, Value);
            
            InsertChannel(Output, OutputSize, InputIndex);
            InsertChannel(Input, InputSize, InputIndex);
            
            ChanOutInt32(Output[InputIndex], Value);
            /*}}}*/
        }
        /*}}}*/
    }
